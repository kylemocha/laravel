...
<script>
  ...
  <script>
    ...
    const calendar1 = new Date();
    const calendar1Control = {
      localDate: new Date(),
      ...
      firstDay: function () {
        return new Date(calendar1.getFullYear(), calendar1.getMonth(), 1);
      },
      lastDay: function () {
        return new Date(calendar1.getFullYear(), calendar1.getMonth() + 1, 0);
      },
      firstDayNumber: function () {
        return calendar1Control.firstDay().getDay() + 1;
      },
      lastDayNumber: function () {
        return calendar1Control.lastDay().getDay() + 1;
      },
      getPreviousMonthLastDate: function () {
        let lastDate = new Date(
          calendar1.getFullYear(),
          calendar1.getMonth(),
          0
        ).getDate();
        return lastDate;
      ...
      navigateToPreviousMonth: function () {
        ...
        calendar1Control.attachEventsOnNextPrev();
      },
      ...
      navigateToCurrentMonth: function () {
        let currentMonth = calendar1Control.localDate.getMonth();
        let currentYear = calendar1Control.localDate.getFullYear();
        calendar1.setMonth(currentMonth);
        calendar1.setYear(currentYear);
        calendar1Control.attachEventsOnNextPrev();
      },
      displayYear: function () {
        let yearLabel = document.querySelector(".calendar1 .calendar1-year-label");
        yearLabel.innerHTML = calendar1.getFullYear();
      },
      displayMonth: function () {
        let monthLabel = document.querySelector(
          ".calendar1 .calendar1-month-label"
        );
        monthLabel.innerHTML = calendar1Control.calMonthName[calendar1.getMonth()];
      },
      selectDate: function (e) {
        console.log(
          `${e.target.textContent} ${
            calendar1Control.calMonthName[calendar1.getMonth()]
          } ${calendar1.getFullYear()}`
        );
      },
      plotSelectors: function () {
        document.querySelector(
          ".calendar1"
        ).innerHTML += `<div class="calendar1-inner"><div class="calendar1-controls">
          <div class="calendar1-prev"><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128"><path fill="#666" d="M88.2 3.8L35.8 56.23 28 64l7.8 7.78 52.4 52.4 9.78-7.76L45.58 64l52.4-52.4z"/></svg></a></div>
          <div class="calendar1-year-month">
          <div class="calendar1-month-label"></div>
          <div>-</div>
          <div class="calendar1-year-label"></div>
          </div>
          <div class="calendar1-next"><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128"><path fill="#666" d="M38.8 124.2l52.4-52.42L99 64l-7.77-7.78-52.4-52.4-9.8 7.77L81.44 64 29 116.42z"/></svg></a></div>
          </div>
          <div class="calendar1-today-date">Today: 
            ${calendar1Control.calWeekDays[calendar1Control.localDate.getDay()]}, 
            ${calendar1Control.localDate.getDate()}, 
            ${calendar1Control.calMonthName[calendar1Control.localDate.getMonth()]} 
            ${calendar1Control.localDate.getFullYear()}
          </div>
          <div class="calendar1-body"></div></div>`;
      },
      plotDayNames: function () {
        for (let i = 0; i < calendar1Control.calWeekDays.length; i++) {
          document.querySelector(
            ".calendar1 .calendar1-body"
          ).innerHTML += `<div>${calendar1Control.calWeekDays[i]}</div>`;
        }
      },
      plotDates: function () {
        document.querySelector(".calendar1 .calendar1-body").innerHTML = "";
        calendar1Control.plotDayNames();
        calendar1Control.displayMonth();
        calendar1Control.displayYear();
        let count = 1;
        let prevDateCount = 0;
  
        calendar1Control.prevMonthLastDate = calendar1Control.getPreviousMonthLastDate();
        let prevMonthDatesArray = [];
        let calendar1Days = calendar1Control.daysInMonth(
          calendar1.getMonth() + 1,
          calendar1.getFullYear()
        );
        // dates of current month
        for (let i = 1; i < calendar1Days; i++) {
          if (i < calendar1Control.firstDayNumber()) {
            prevDateCount += 1;
            document.querySelector(
              ".calendar1 .calendar1-body"
            ).innerHTML += `<div class="prev-dates"></div>`;
            prevMonthDatesArray.push(calendar1Control.prevMonthLastDate--);
          } else {
            document.querySelector(
              ".calendar1 .calendar1-body"
        ...
        calendar1Control.highlightToday();
        calendar1Control.plotPrevMonthDates(prevMonthDatesArray);
        calendar1Control.plotNextMonthDates();
      },
      attachEvents: function () {
        let prevBtn = document.querySelector(".calendar1 .calendar1-prev a");
        let nextBtn = document.querySelector(".calendar1 .calendar1-next a");
        let todayDate = document.querySelector(".calendar1 .calendar1-today-date");
        let dateNumber = document.querySelectorAll(".calendar1 .dateNumber");
        prevBtn.addEventListener(
          "click",
          calendar1Control.navigateToPreviousMonth
        );
        nextBtn.addEventListener("click", calendar1Control.navigateToNextMonth);
        todayDate.addEventListener(
          "click",
          calendar1Control.navigateToCurrentMonth
        );
        for (var i = 0; i < dateNumber.length; i++) {
            dateNumber[i].addEventListener(
              "click",
              calendar1Control.selectDate,
              false
            );
        }
      },
      highlightToday: function () {
        let currentMonth = calendar1Control.localDate.getMonth() + 1;
        let changedMonth = calendar1.getMonth() + 1;
        let currentYear = calendar1Control.localDate.getFullYear();
        let changedYear = calendar1.getFullYear();
        if (
          ...
          document
            ...
            [calendar1.getDate() - 1].classList.add("calendar1-today");
        }
      },
      ...
      plotNextMonthDates: function(){
       let childElemCount = document.querySelector('.calendar1-body').childElementCount;
       //7 lines
       if(childElemCount > 42 ) {
           let diff = 49 - childElemCount;
           calendar1Control.loopThroughNextDays(diff);
       }

       //6 lines
       if(childElemCount > 35 && childElemCount <= 42 ) {
        let diff = 42 - childElemCount;
        calendar1Control.loopThroughNextDays(42 - childElemCount);
       }
      ...
      loopThroughNextDays: function(count) {
        if(count > 0) {
            for(let i=1;i<=count;i++) {
                document.querySelector('.calendar1-body').innerHTML += `<div class="next-dates">${i}</div>`;
            }
        }
      },
      attachEventsOnNextPrev: function () {
        calendar1Control.plotDates();
        calendar1Control.attachEvents();
      },
      init: function () {
        calendar1Control.plotSelectors();
        calendar1Control.plotDates();
        calendar1Control.attachEvents();
      }
    };
    calendar1Control.init();
  }
  
  const calendar1Control = new 1();
  </script>-->
